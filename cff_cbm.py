...
class SGFM(nn.Module):
    ...
    def sgfm(self, img_feat, flow_feat, conv1x1_channel_wise, conv1x1_spatial):
        # spatial attention
    ...
    def forward(self, img_feats, flow_feats):  # 8-256-96-96,,8-512-48-48,,8-1024-24-24,,8-1024-12-12

        img_feats['s32'] = self.sgfm(img_feats['s32'], flow_feats['s32'], self.conv1x1_layer4_channel_wise,
                                                  self.conv1x1_layer4_spatial)
        return img_feats
    ...
    def forward(self, imgs, flows):
        ...
        for i in range(L):
            if B != 1 or not self.aos:
                ...
                # 级联增强模块
                app_feats_cb = self.vos.cbm(app_feats, mo_feats)
                final_score = self.vos.decoder(app_feats_cb, mo_feats)
            ...
            if B == 1 and self.aos:
                ...
                app_feats_cb = self.vos.cbm(app_feats, mo_feats_flow)
                app_feats_cb['s32'] = app_feats_cb['s32'] + app_feats['s32']
                final_score_img = self.vos.decoder(app_feats_cb, mo_feats_img)
                final_score_flow = self.vos.decoder(app_feats_cb, mo_feats_flow)
