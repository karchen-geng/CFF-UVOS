...
from utils.spent_time import TimeSpent
from utils.utils import AverageMeter, get_iou
import copy
import numpy
import torch
...
class Trainer(object):
    def __init__(self, model, ver, optimizer, train_loader, val_set, date, save_name, save_step, val_step):
    ...
    def train(self, max_epochs):
        ...
        for epoch in range(self.epoch, max_epochs + 1):
            ...
            ep_time_str = "Epotch {}:总用时 {}天 {:.0f}小时 {:.0f}分钟 {:.4f}秒".format(epoch, epotch_time.days,
    ...
    def save_checkpoint(self, alt_name=None):
        if alt_name is not None:
            file_path = 'weights/{}/{}_{}.pth'.format(self.date, self.save_name, alt_name)
        else:
            file_path = 'weights/{}/{}_{:04d}.pth'.format(self.date, self.save_name, self.epoch)
