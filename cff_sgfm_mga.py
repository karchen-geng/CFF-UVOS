...
class MGA(nn.Module):
    ...
    def forward(self, img_feats, flow_feats):  # 8-256-96-96,,8-512-48-48,,8-1024-24-24,,8-1024-12-12
        ...
        img_feats['s32'] = self.encoder_attention_module_MGA_tmc(img_feats['s32'], flow_feats['s32'], self.conv1x1_layer4_channel_wise,
    ...
    def forward(self, imgs, flows):
        ...
        for i in range(L):
            ...
            if B != 1 or not self.aos:
                ...
                # 级联增强模块
                app_feats_mix = self.vos.mga_module(app_feats, mo_feats)
            ...
            if B == 1 and self.aos:
                ...
                app_feats_mix = self.vos.mga_module(app_feats, mo_feats_flow)
